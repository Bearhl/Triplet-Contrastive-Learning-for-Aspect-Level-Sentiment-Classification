...
from data_utils import ParseData, reshape_tree
...
def main():
    ...
    # dep_tag
    vocab_dep_tag_file = args.vocab_dir + '/vocab_dep_tag.vocab'
    # polarity
    ...
    train_dep_tag = []
    test_dep_tag = []
    for obj in ParseData(train_file):
        train_dep_tag.extend(reshape_tree(obj))
    for obj in ParseData(test_file):
        test_dep_tag.extend(reshape_tree(obj))
    ...
    dep_tag_counter = Counter(train_dep_tag + test_dep_tag)
    ...
    dep_tag_vocab = VocabHelp(dep_tag_counter, specials=['<pad>', '<unk>'])
    post_vocab = VocabHelp(post_counter, specials=['<pad>', '<unk>'])
    pol_vocab = VocabHelp(pol_counter, specials=[])
    print("token_vocab: {}, pos_vocab: {}, dep_vocab: {}, dep_tag_vocab: {}, post_vocab: {}, pol_vocab: {}".format(
        len(token_vocab), len(pos_vocab), len(dep_vocab), len(dep_tag_vocab), len(post_vocab), len(pol_vocab)))

    print("dumping to files...")
    token_vocab.save_vocab(vocab_tok_file)
    pos_vocab.save_vocab(vocab_pos_file)
    dep_vocab.save_vocab(vocab_dep_file)
    dep_tag_vocab.save_vocab(vocab_dep_tag_file)
